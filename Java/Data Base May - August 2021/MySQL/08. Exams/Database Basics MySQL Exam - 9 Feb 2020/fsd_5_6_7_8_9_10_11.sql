...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `udp_find_playmaker`(min_dribble_points INT, team_name VARCHAR (45))
BEGIN

SELECT concat_ws(' ', p.first_name, p.last_name) AS `full_name`, p.age, p.salary, sk.dribbling, sk.speed AS `speed`, t.`name`
FROM players AS p
JOIN teams AS t
ON p.team_id = t.id
JOIN skills_data AS sk
