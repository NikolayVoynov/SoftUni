...
DELIMITER $$
CREATE PROCEDURE `usp_get_towns_starting_with`(string_parameter VARCHAR(20))
BEGIN
SELECT name AS town_name
FROM towns
WHERE substring(name, 1, length(string_parameter)) = string_parameter
ORDER BY town_name;
END$$
...
DELIMITER $$
CREATE FUNCTION `ufn_get_salary_level`(employee_salary DECIMAL) RETURNS varchar(100) CHARSET utf8
    DETERMINISTIC
BEGIN
DECLARE level_salary VARCHAR(100);
SET level_salary := (
if(employee_salary < 30000, 'Low', if(employee_salary BETWEEN 30000 AND 50000, 'Average', if(employee_salary > 50000, 'High', NULL)))
);
RETURN level_salary;
END$$
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_get_employees_by_salary_level`(salary_level VARCHAR(50))
BEGIN
...
WHERE
(
if(salary_level = 'Low', salary < 30000, 
if(salary_level = 'Average', salary BETWEEN 30000 AND 50000, 
if(salary_level = 'High', salary > 50000, NULL)))
)
