...
import com.example.spring_intro.model.entity.*;
import com.example.spring_intro.repository.BookRepository;
...
public class BookServiceImpl implements BookService {
    ...
    public void seedBooks() throws IOException {
        ...
        Files.readAllLines(Path.of(BOOKS_FILE_PATH))
                .forEach(row -> {
                    ...
                    Book book = createBookWithReceivedInfo(bookInfo);
    ...
    @Override
    public List<String> findAllBookTittlesWithAgeRestriction(AgeRestriction ageRestriction) {
    ...
    @Override
    public List<String> findAllBooksTitlesAndPricesWithPriceLessThan5OrMoreThan40() {
        return bookRepository
    ...
    @Override
    public List<String> findBookTitlesNotReleasedInYear(int year) {
    ...
    @Override
    public List<String> findAllBookTitlesWhichContainString(String containedString) {
        return bookRepository
                .findByTitleContains(containedString)
                .stream()
    ...
    @Override
    public List<String> findAllTitleWithAuthorLastNameStartsWith(String startString) {
        return bookRepository
    ...
    @Override
    public int findNumberOfAllBookTitlesLengthLongerThan(int givenNumber) {
        return bookRepository
                .countOfBooksWithTitleLengthMoreThan(givenNumber);
    }

    @Override
    public String retrieveInformationForBookWithTitle(String bookTitle) {
        Book searchedBook = bookRepository.findBookByTitle(bookTitle);

        return String.format("%s %s %s %.2f",
                searchedBook.getTitle(),
                searchedBook.getEditionType().name(),
                searchedBook.getAgeRestriction().name(),
                searchedBook.getPrice());
    }

    private Book createBookWithReceivedInfo(String[] bookInfo) {
